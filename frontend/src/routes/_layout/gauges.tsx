...
function Dashboard() {
  ...
  const handleStationsChange = (details: Combobox.ValueChangeDetails) => {
    ...
    navigate({
      search: (prev: any) => ({ ...prev, stations: new_selection.join(',') }),
    })
  }
  ...
  const handleYearChange = (details: Select.ValueChangeDetails) => {
    ...
    navigate({
      search: (prev: any) => ({
        year: Number(details.value),
        stations: prev.stations.join(','),
      }),
    })
  }
      ...
      <Box maxH={'100vh'} pt={12} m={4}>
        ...
        >
          <Flex justifyContent={'flex-end'} align={'end'} gap={5}>
            <YearSelect year={year.toString()} onChange={handleYearChange} />{' '}
            <StationSelect
