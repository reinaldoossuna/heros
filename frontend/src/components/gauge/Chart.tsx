...
const Chart = ({ station, year }: chartPropTypes) => {
  ...
  const { data, error, status } = useQuery({
    ...getDailyStationDataApiGaugesDailyStationGetOptions({
      ...
      query: { start: new Date(year, 0, 1), end: new Date(year + 1, 0, 1) },
    }),
    retry: false,
  })
  ...
  return (
    ...
    >
      <Heading size={'sm'} marginBottom={'1rem'}>
        ...
        {status === 'pending' && <Spinner color="teal.500" size="lg" />}
      ...
      {status === 'success' && data.length > 0 && (
        ...
        >
          ...
          <XAxis
            ...
            domain={[
              moment(`01/01/${year}`).dayOfYear(),
              moment(`31/12/${year}`).dayOfYear(),
            ]}
            ticks={[...Array(12).keys()]
              .map((i) => i + 1)
              .map((m) => moment(`${year}/${m}/01`).dayOfYear())}
            tickFormatter={(value, _) =>
              moment(`01/01/${year}`).dayOfYear(value).format('MMM')
            }
            scale="time"
            type="number"
          />
          ...
          <Tooltip
            ...
            labelFormatter={(value) =>
              moment(`01/01/${year}`).dayOfYear(value).format('DD/MM/YYYY')
            }
          />
          <Line
            yAxisId={'accumulated'}
