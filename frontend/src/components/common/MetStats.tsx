...
const filter_data = (
  data: Array<MetereologicoData>,
  key: keyof MetereologicoData
) => {
  /* Make TS happy */
  if (key === 'data') return []
...
const MetStats = ({ data }: { data: Array<MetereologicoData> }) => {
  ...
  const rain_acc = filter_data(data, 'precipitacao').reduce(
    (acc, v) => acc + v,
    0
  )
  ...
  return (
    <>
      <Stat
        ...
        color={Status.Ok}
      >{`${mean_atm.toFixed(1)} bar`}</Stat>
      <Stat
        icon={IoThermometerSharp}
        title="Temp. Media"
        color={Status.Ok}
      >{`${mean_temp.toFixed(1)} °C`}</Stat>
      <Stat
        icon={GiRaining}
        title="Chuva Acumulada"
        color={Status.Ok}
      >{`${rain_acc.toFixed(1)} mm`}</Stat>
      <Stat
        icon={GiWaterDrop}
        title="Umidade do ar"
        color={Status.Ok}
      >{`${mean_rh.toFixed(1)} %`}</Stat>
    </>
  )
}
